#!/usr/bin/env python3
"""
UPI Giveaway 2026 Bot - v22+ with Native Colored Buttons & Premium Emojis
Telegram Bot API 9.4+ features: button styles (success/danger/primary) + custom emoji icons
"""
...
from telegram import (
    ...
    KeyboardButtonStyle,  # NEW: For button colors
    BotCommand
)
...
# Premium Emoji IDs for MESSAGE TEXT and BUTTON ICONS
EMOJI_IDS = [
...
BUTTON_STYLES = {
    "success": KeyboardButtonStyle.SUCCESS,   # 🟢 Green
    "danger": KeyboardButtonStyle.DANGER,     # 🔴 Red
    "primary": KeyboardButtonStyle.PRIMARY,   # 🔵 Blue
    "default": None                           # Default gray
}
...
def get_style(style_name: str):
    """Get KeyboardButtonStyle from string name"""
    return BUTTON_STYLES.get(style_name, KeyboardButtonStyle.PRIMARY)
...
def create_channel_buttons(channels: list) -> List[List[InlineKeyboardButton]]:
    ...
    for i, ch in enumerate(channels):
        style_name = ch.get("style", styles[i % len(styles)])
        style = get_style(style_name)

        # Use channel's emoji_id or get a random one
        emoji_id = ch.get("emoji_id") or get_random_emoji_id()

        btn = InlineKeyboardButton(
            text=ch['name'],
            url=ch['link'],
            style=style,                    # NATIVE BUTTON COLOR!
            icon_custom_emoji_id=emoji_id   # PREMIUM EMOJI ICON!
        )
        if i % 2 == 0:
            buttons.append([btn])
        else:
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Add premium emoji to caption using HTML
    caption += f'\n\n<tg-emoji emoji-id="{emoji_id}">⭐</tg-emoji> Join all channels then click on Joined!'

    # Build colored channel buttons with premium emoji icons
    channel_buttons = create_channel_buttons(page1_channels)

    # Add "Joined" button - GREEN (Success) with premium emoji
    joined_emoji = get_random_emoji_id()
    joined_btn = InlineKeyboardButton(
        text="Joined",
        callback_data="check_joined",
        style=KeyboardButtonStyle.SUCCESS,        # 🟢 GREEN BUTTON!
        icon_custom_emoji_id=joined_emoji          # Premium emoji icon!
    )
...
async def check_joined_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if all_joined:
        ...
        caption = f'<tg-emoji emoji-id="{emoji_id}">⭐</tg-emoji> Join these channels also, then click Recharge!'
        ...
        recharge_btn = InlineKeyboardButton(
            text="Recharge",
            callback_data="check_recharge",
            style=KeyboardButtonStyle.PRIMARY,       # 🔵 BLUE BUTTON!
            icon_custom_emoji_id=recharge_emoji        # Premium emoji icon!
        )
        ...
        emoji_id = get_random_emoji_id()
        caption = f'❌ You haven\'t joined all channels yet!\n\n<tg-emoji emoji-id="{emoji_id}">⭐</tg-emoji> Please join these channels:'
        ...
        joined_btn = InlineKeyboardButton(
            ...
            icon_custom_emoji_id=joined_emoji
        )
...
async def check_recharge_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if page2_channels:
        ...
        if not all_joined:
            ...
            recharge_btn = InlineKeyboardButton(
                ...
                icon_custom_emoji_id=recharge_emoji
            )
    ...
    if ref_count >= target:
        ...
        caption = (
            f'<tg-emoji emoji-id="{emoji_id}">⭐</tg-emoji> Referral System\n\n'
