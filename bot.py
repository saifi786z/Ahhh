#!/usr/bin/env python3
"""
UPI Giveaway 2026 Bot - Fixed Version
Premium Emoji & Colored Buttons Support
"""
...
BOT_USERNAME = "UPIGIVEAWAY2026_BOT"  # From your screenshot

# Premium Emoji IDs (Telegram custom emoji)
# These are paid Telegram emoji IDs - they work with HTML parse mode
EMOJI_IDS = [
...
# Button color emojis (since Telegram API doesn't support colored inline buttons directly)
BUTTON_STYLES = {
    "success": "🟢",   # Green
    "danger": "🔴",    # Red  
    "primary": "🔵",   # Blue
...
def add_channel(page: str, name: str, link: str, channel_id: str = None, style: str = "primary"):
...
def get_random_emoji() -> str:
    """Get a random premium emoji using HTML tag format"""
    emoji_id = random.choice(EMOJI_IDS)
    # Use the <tg-emoji> HTML tag format for premium emojis
    return f'<tg-emoji emoji-id="{emoji_id}">⭐</tg-emoji>'

def get_button_style_emoji(style: str) -> str:
    """Get button style color emoji"""
    return BUTTON_STYLES.get(style, "🔵")

def create_channel_buttons(channels: list, page: str) -> List[List[InlineKeyboardButton]]:
    """Create channel link buttons in rows of 2 with colored styles"""
    buttons = []
    styles = ["success", "danger", "primary"]

    for i, ch in enumerate(channels):
        style = ch.get("style", styles[i % len(styles)])
        style_emoji = get_button_style_emoji(style)

        btn = InlineKeyboardButton(
            text=f"{style_emoji} {ch['name']}",
            url=ch['link']
        )
        if i % 2 == 0:
            buttons.append([btn])
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Build caption with premium emoji
    ...
    # Build channel buttons with colors
    channel_buttons = create_channel_buttons(page1_channels, "page1")

    # Add Joined button with style
    joined_emoji = get_button_style_emoji("success")
    joined_button = [InlineKeyboardButton(f"{joined_emoji} Joined", callback_data="check_joined")]
...
async def check_joined_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if all_joined:
        ...
        recharge_emoji = get_button_style_emoji("primary")
        recharge_button = [InlineKeyboardButton(f"{recharge_emoji} Recharge", callback_data="check_recharge")]
...
async def admin_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "admin_back":
        ...
        await query.edit_message_text(
            ...
            "<code>Name | Link | ChannelID | Style</code>\n\n"
            "Styles: success(🟢), danger(🔴), primary(🔵), warning(🟡)\n"
            "Example:\n"
            "<code>My Channel | https://t.me/mychannel | @mychannel | success</code>\n\n"
        ...
        await query.edit_message_text(
            ...
            "Styles: success(🟢), danger(🔴), primary(🔵), warning(🟡)\n"
            "Example:\n"
            "<code>My Channel | https://t.me/mychannel | @mychannel | danger</code>\n\n"
            "Or send /cancel to abort.",
            parse_mode=ParseMode.HTML
        )
        return ADD_CHANNEL
        ...
        for i, ch in enumerate(page1):
            style_emoji = get_button_style_emoji(ch.get("style", "primary"))
            text += f"{i+1}. {style_emoji} {ch['name']} - {ch['link']}\n"
        ...
        for i, ch in enumerate(channels.get("page1", []), 1):
            style_emoji = get_button_style_emoji(ch.get("style", "primary"))
            text += f"{i}. {style_emoji} {ch['name']} - {ch['link']}\n"
...
async def add_channel_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        add_channel(page, name, link, channel_id, style)
        style_emoji = get_button_style_emoji(style)
        ...
        await update.message.reply_text(
            f"❌ Error: {e}\n\n"
            "Please use format: <code>Name | Link | ChannelID | Style</code>",
            parse_mode=ParseMode.HTML
        )
