#!/usr/bin/env python3
"""
UPI Giveaway 2026 Bot - With New User Notification + Count
"""
...
BOT_USERNAME = "UPIGIVEAWAY2026_BOT"

# Premium Emoji IDs for MESSAGE TEXT (HTML tg-emoji tags) and BUTTON ICONS
EMOJI_IDS = [
...
def add_channel(page: str, name: str, link: str, channel_id: str = None, style: str = "primary", emoji_id: str = None):
...
async def send_new_user_notification(bot, new_user_id: int, username: str = None, first_name: str = None):
    """Send new user notification to all admins with count"""
    ...
    # Build notification message with premium emojis
    emoji1 = get_random_emoji_id()
    emoji2 = get_random_emoji_id()
    emoji3 = get_random_emoji_id()

    # User info
    user_mention = f"@{username}" if username else f"[{first_name or 'Unknown'}](tg://user?id={new_user_id})"

    notification_text = (
        f'<tg-emoji emoji-id="{emoji1}">🎉</tg-emoji> <b>New User Joined!</b> <tg-emoji emoji-id="{emoji2}">🎉</tg-emoji>\n\n'
        f'👤 <b>User:</b> {user_mention}\n'
        f'🆔 <b>User ID:</b> <code>{new_user_id}</code>\n'
        f'\n'
        f'<tg-emoji emoji-id="{emoji3}">📊</tg-emoji> <b>Total Users:</b> <code>{total_count}</code>\n'
        f'✅ <b>User #{total_count}</b> joined the bot!'
    )
    ...
    notifications["total_users_notified"] = notifications.get("total_users_notified", 0) + 1
    save_notifications(notifications)

# ==================== EMOJI HELPERS ====================

def get_random_emoji_id() -> str:
    """Get a random premium emoji ID"""
    return random.choice(EMOJI_IDS)

def pe(emoji_id: str = None) -> str:
    """Premium emoji HTML tag for message text"""
    eid = emoji_id or get_random_emoji_id()
    return f'<tg-emoji emoji-id="{eid}">⭐</tg-emoji>'

# ==================== COLORED BUTTON BUILDER ====================

def styled_button(text: str, url: str = None, callback_data: str = None, 
                  style: str = "primary", emoji_id: str = None) -> InlineKeyboardButton:
    """Create a button with native color and premium emoji icon using api_kwargs"""
    ...
    if url:
        ...
        return InlineKeyboardButton(text=text, api_kwargs=api_kwargs)

def create_channel_buttons(channels: list) -> List[List[InlineKeyboardButton]]:
    """Create colored channel link buttons in rows of 2 with premium emoji icons"""
    buttons = []
    styles = ["success", "danger", "primary"]

    for i, ch in enumerate(channels):
        style = ch.get("style", styles[i % len(styles)])
        emoji_id = ch.get("emoji_id") or get_random_emoji_id()
...
def get_admin_keyboard() -> InlineKeyboardMarkup:
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("🔔 Toggle Notifications", callback_data="admin_toggle_notif")],
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Add user and check if new
    is_new_user = add_user(user_id, user.username, user.first_name)

    # Send notification to admins if new user
    if is_new_user:
    ...
    # Premium emoji for caption
    emoji_id = get_random_emoji_id()

    # Build caption with premium emoji EVERYWHERE
    caption = settings.get("start_caption", "Welcome to UPI Giveaway 2026!")
    caption = f"{pe(emoji_id)} {caption} {pe()}"
    caption += f"\n\n{pe()} Join all channels then click on Joined! {pe()}"

    # Build colored channel buttons with premium emoji icons
    channel_buttons = create_channel_buttons(page1_channels)

    # Add "Joined" button - GREEN (success) with premium emoji icon
    joined_emoji = get_random_emoji_id()
    joined_btn = styled_button(
        text="Joined",
        callback_data="check_joined",
        style="success",
        emoji_id=joined_emoji
    )

    keyboard = channel_buttons + [[joined_btn]]
...
async def check_joined_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not page1_channels:
        await query.edit_message_text(
            f"{pe()} No channels configured. Please contact admin.",
            parse_mode=ParseMode.HTML
        )
        return
    ...
    if all_joined:
        ...
        caption = f"{pe()} Join these channels also, then click Recharge! {pe()}"

        channel_buttons = create_channel_buttons(page2_channels)

        # Recharge button - BLUE (primary) with premium emoji
        recharge_emoji = get_random_emoji_id()
        recharge_btn = styled_button(
            text="Recharge",
            callback_data="check_recharge",
            style="primary",
            emoji_id=recharge_emoji
        )

        keyboard = channel_buttons + [[recharge_btn]]
        reply_markup = InlineKeyboardMarkup(keyboard)
        ...
        caption = f"❌ {pe()} You haven't joined all channels yet! {pe()}\n\n{pe()} Please join these channels: {pe()}"

        channel_buttons = create_channel_buttons(not_joined)
        joined_emoji = get_random_emoji_id()
        joined_btn = styled_button(
            text="Joined",
            callback_data="check_joined",
...
async def check_recharge_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if ref_count >= target:
        caption = (
            f"{pe()} 🎉 Congratulations! 🎉 {pe()}\n\n"
            f"You have completed {ref_count}/{target} referrals!\n\n"
            f"✅ Please contact @{contact} for your profit!"
        )
        keyboard = [[InlineKeyboardButton("📞 Contact @" + contact, url=f"https://t.me/{contact}")]]
    else:
        remaining = target - ref_count
        caption = (
            f"{pe()} Referral System {pe()}\n\n"
            f"📊 Your Referrals: {ref_count}/{target}\n"
            f"⏳ Remaining: {remaining}\n\n"
            f"{pe()} Share your referral link to get {target} referrals and claim your reward! {pe()}"
        )
...
async def admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_admin(user_id):
        await update.message.reply_text(f"{pe()} ❌ You are not authorized to access the admin panel.")
        return

    await update.message.reply_text(
        f"{pe()} 🔧 <b>Admin Panel</b> {pe()}\n\nWelcome to the admin dashboard!",
...
async def admin_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_admin(user_id):
        await query.edit_message_text(f"{pe()} ❌ Unauthorized access.")
        return
    ...
    if data == "admin_back":
        ...
        notifications = get_notifications()
        notifications["enabled"] = not notifications.get("enabled", True)
        save_notifications(notifications)

        status = "ON ✅" if notifications["enabled"] else "OFF ❌"
        total_notified = notifications.get("total_users_notified", 0)

        await query.edit_message_text(
            f"{pe()} 🔔 <b>Notification Settings</b> {pe()}\n\n"
            f"Status: {status}\n"
            f"Total Users Notified: <code>{total_notified}</code>\n\n"
        ...
        await query.edit_message_text(
            f"{pe()} ➕ <b>Add Channel to Page 1</b> {pe()}\n\n"
            "Send channel details in format:\n"
            "<code>Name | Link | ChannelID | Style | EmojiID</code>\n\n"
            "Styles: success(🟢), danger(🔴), primary(🔵)\n"
            "Example:\n"
            "<code>My Channel | https://t.me/mychannel | @mychannel | success | 6023898164732366954</code>\n\n"
        ...
        await query.edit_message_text(
            f"{pe()} ➕ <b>Add Channel to Page 2</b> {pe()}\n\n"
            "Send channel details in format:\n"
            "<code>Name | Link | ChannelID | Style | EmojiID</code>\n\n"
            "Styles: success(🟢), danger(🔴), primary(🔵)\n"
            "Example:\n"
            "<code>My Channel | https://t.me/mychannel | @mychannel | danger | 6026106482297147601</code>\n\n"
            "Or send /cancel to abort.",
            parse_mode=ParseMode.HTML
        )
        return ADD_CHANNEL
        ...
        if not page1:
            await query.edit_message_text(
                f"{pe()} ❌ No channels in Page 1.",
                reply_markup=get_admin_keyboard(),
                parse_mode=ParseMode.HTML
            )
            return

        text = f"{pe()} ➖ <b>Remove Channel from Page 1</b> {pe()}\n\n"
        keyboard = []
        for i, ch in enumerate(page1):
            text += f"{i+1}. [{ch.get('style', 'primary').upper()}] {ch['name']} - {ch['link']}\n"
        ...
        if not page2:
            await query.edit_message_text(
                f"{pe()} ❌ No channels in Page 2.",
                reply_markup=get_admin_keyboard(),
                parse_mode=ParseMode.HTML
            )
            return

        text = f"{pe()} ➖ <b>Remove Channel from Page 2</b> {pe()}\n\n"
        keyboard = []
        ...
        if remove_channel("page1", index):
            await query.edit_message_text(
                f"{pe()} ✅ Channel removed from Page 1!",
                reply_markup=get_admin_keyboard(),
                parse_mode=ParseMode.HTML
            )
        else:
            await query.edit_message_text(
                f"{pe()} ❌ Failed to remove channel.",
                reply_markup=get_admin_keyboard(),
                parse_mode=ParseMode.HTML
            )
        ...
        if remove_channel("page2", index):
            await query.edit_message_text(
                f"{pe()} ✅ Channel removed from Page 2!",
                reply_markup=get_admin_keyboard(),
                parse_mode=ParseMode.HTML
            )
        else:
        ...
        await query.edit_message_text(
            f"{pe()} ✏️ <b>Set Start Caption</b> {pe()}\n\n"
        ...
        if remove_admin(admin_id):
            await query.edit_message_text(
                f"{pe()} ✅ Admin {admin_id} removed!",
                reply_markup=get_admin_keyboard(),
                parse_mode=ParseMode.HTML
            )
        else:
            await query.edit_message_text(
                f"{pe()} ❌ Failed to remove admin.",
                reply_markup=get_admin_keyboard(),
                parse_mode=ParseMode.HTML
            )
        ...
        text = (
            f"{pe()} 📊 <b>Bot Statistics</b> {pe()}\n\n"
            f"👥 Total Users: <code>{total_users}</code>\n"
            f"🔗 Total Referrals: <code>{total_refs}</code>\n"
            f"✅ Completed Users: <code>{completed_users}</code>\n"
            f"🔔 Notifications Sent: <code>{total_notified}</code>\n"
            f"📢 Page 1 Channels: <code>{len(channels.get('page1', []))}</code>\n"
            f"📢 Page 2 Channels: <code>{len(channels.get('page2', []))}</code>\n"
        )
...
async def add_channel_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        style = parts[3].strip() if len(parts) > 3 else "primary"
        emoji_id = parts[4].strip() if len(parts) > 4 else None

        add_channel(page, name, link, channel_id, style, emoji_id)

        await update.message.reply_text(
            f"{pe()} ✅ Channel added to {page}! {pe()}\n\n"
            f"Name: {name}\n"
            f"Link: {link}\n"
            f"Style: {style.upper()}\n"
            f"Emoji ID: {emoji_id or 'Random'}",
            reply_markup=get_admin_keyboard(),
            parse_mode=ParseMode.HTML
        )
    except Exception as e:
        await update.message.reply_text(
            f"{pe()} ❌ Error: {e}\n\n"
            "Please use format: <code>Name | Link | ChannelID | Style | EmojiID</code>",
            parse_mode=ParseMode.HTML
        )
...
async def set_start_image_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message.photo:
        ...
        await update.message.reply_text(
            f"{pe()} ✅ Start image set successfully!",
            reply_markup=get_admin_keyboard(),
            parse_mode=ParseMode.HTML
        )
    else:
        await update.message.reply_text(
            f"{pe()} ❌ Please send an image. Try again or send /cancel.",
            parse_mode=ParseMode.HTML
        )
        return SET_START_IMAGE
...
async def broadcast_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"{pe()} 📢 Broadcasting... Please wait.")
...
async def add_admin_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if add_admin(new_admin_id):
            await update.message.reply_text(
                f"{pe()} ✅ Admin {new_admin_id} added successfully!",
                reply_markup=get_admin_keyboard(),
                parse_mode=ParseMode.HTML
            )
        else:
            await update.message.reply_text(
                f"{pe()} ❌ User is already an admin.",
                reply_markup=get_admin_keyboard(),
                parse_mode=ParseMode.HTML
            )
    except ValueError:
        await update.message.reply_text(
            f"{pe()} ❌ Invalid User ID. Please send a valid number.",
            reply_markup=get_admin_keyboard(),
            parse_mode=ParseMode.HTML
        )
    ...
    if update and update.effective_message:
        try:
            await update.effective_message.reply_text(
                f"{pe()} ❌ An error occurred. Please try again later."
            )
        except:
            pass
